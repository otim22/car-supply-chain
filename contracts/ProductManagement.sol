...
contract ProductManagement {
    ...
    function concatenateInfoAndHash(address a1, string memory s1, string memory s2, string memory s3) pure private returns (bytes32) {
        ...
        // Then calculate and reverse a space for the full string
    ...
    function buildProduct(string memory serial_number, string memory product_type, string memory creation_date, bytes32[6] memory part_array) public returns (bytes32) {
        ...
        // Create hash for data and check if exists. If it doesn't, create the part and return return the ID to the user
