...
contract ProductManagement {
    ...
    function getParts(bytes32 product_hash) public pure returns (bytes32[6] memory) {}
}
...
contract ChangeOwnership {
    ...
    constructor(address prod_contract_addr) public {
        // Just create a new auxillary contract. we will use it to check if the part or product really exist
    ...
    function addOwnership(uint op_type, bytes32 p_hash) public returns (bool) {
        if(op_type == uint(OperationType.PART)) {
            ...
            require(manufacturer == msg.sender, "Part was not made by the requestor");
    ...
    function changeOwnership(uint op_type, bytes32 p_hash, address to) public returns (bool) {
        ...
        if(op_type == uint(OperationType.PART)){
            ...
            require(currentProductOwner[p_hash] == msg.sender, "Product is not owned by the requester");
